...
The Hyper server will now listen in the background for incoming requests while we write an output message in parallel that the server is running. ```println!()``` is a macro in Rust (yet nother concept), but you can think of it as just a function call with variable arguments for now.
...
Stay tuned for future blog posts where I plan to explore Configuration file handling, Automated Testing, Structuring larger programs, Benchmarking and more.
